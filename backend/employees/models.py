...
class Employee(models.Model):
    ...
    passportIssuedBy = models.TextField("Кем выдан паспорт", null=True, blank=True)
    passportIssueDate = models.DateField("Дата выдачи паспорта")
    passportValidityPeriod = models.DateField("Срок действия паспорта", null=True, blank=True)
    ...
    phoneNumber = models.CharField("Номер телефона", max_length=40, null=True, blank=True)
    INN = models.CharField("ИНН", max_length=12, validators=[MinLengthValidator(limit_value=12, message="ИНН не может состоят из менее чем 12 цифр"), int_list_validator( sep = '' , message = "ИНН должен содержать только цифры" , code = 'invalid' , allow_negative = False )], null=True, blank=True )
    SNILS = models.CharField("СНИЛС", max_length=11, validators=[MinLengthValidator(limit_value=11, message="СНИЛС не может состоят из менее чем 11 цифр"), int_list_validator( sep = '' , message = "СНИЛС должен содержать только цифры" , code = 'invalid' , allow_negative = False )], null=True, blank=True)
    registrationAddress = models.TextField("Адрес регистрации")
    registrationValidityPeriod = models.DateField("Срок действия регистрации", null=True, blank=True)
    ...
    bankDetailsCardNumber = models.TextField("Номер банковской карты", null=True, blank=True)
    endDateOfRVP = models.DateField("Дата окончания РВП", null=True, blank=True)
    endDateOfResidencePermit = models.DateField("Дата окончания вида на жительство", null=True, blank=True)
...
class EmployeeInOrganization(models.Model):
    ...
    employmentContractDate = models.DateField("Дата трудового договора", null=True, blank=True)
