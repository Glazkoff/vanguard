...
class Patent(models.Model):
    ...
    patentIssuedBy = models.TextField("Кем выдан патент", default="", blank=True)
    ...
    def __str__(self):
        date = self.dateOfPatentIssue.strftime("%d.%m.%Y")
        return f"Патент {self.employee.fullNameInGenetive} от {date}"
...
class PatentPaymentReceipt(models.Model):
    ...
    patent = models.ForeignKey(
        Patent, on_delete=models.CASCADE, verbose_name="Оплаченный патент")
